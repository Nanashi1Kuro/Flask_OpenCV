    ...
    if len(cnts) > 0:
        ...
        for c in cnts:
            ...
            if (area < 40000) and (area > 600):
                ...
                if len(points) >= 2:
                    cv2.line(image, points[0], points[1], (0, 0, 10), 2)
                    cv2.circle(image, points[0], 5, (0, 0, 255), -1)
                    cv2.circle(image, points[1], 5, (0, 0, 255), -1)
                    cv2.line(image, points[0], points[1],
                             (0, 0, 255), 2)

                    # compute the Euclidean distance between the coordinates,
                    # and then convert the distance in pixels to distance in
                    # units
                    D = dist.euclidean(points[0], points[1])
                    (mX, mY) = midpoint(points[0], points[1])
                    cv2.putText(image, "{:.1f}".format(D), (int(mX), int(mY - 10)),
